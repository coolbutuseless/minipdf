...
```{r include = FALSE}
system("magick -density 300 figures/example1.pdf -resize 100% -define png:exclude-chunks=date,time figures/example1.png")
```
...
```{r}
doc <- pdf_doc(width = 400, height = 400, fontname = 'Helvetica-Bold')
...
doc$text("minipdf", x = 95, y = 20, fontsize = 65, stroke = 'grey50', fill = 'lightblue', text_mode = 2)
...
```{r include = FALSE}
system("magick -density 300 figures/example2.pdf -resize 100% -define png:exclude-chunks=date,time figures/example2.png")
```
