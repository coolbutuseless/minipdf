...
Use the `add_rect()` method on a `PDFDocument` object. This has the side effect of
...
Use the `add_dict()` method on a `PDFDocument` object. This has the side effect of
also adding the newly created dict to the document itself.
